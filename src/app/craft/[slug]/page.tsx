      ...
      <div className="w-full max-w-3xl px-6 py-24 flex flex-col gap-16">
        ...
        <header className="flex flex-col gap-12">
          ...
          <div className="flex flex-col gap-3">
            <nav className="flex items-center gap-2 text-[9px] uppercase tracking-[0.25em] text-muted-foreground/30 font-bold mb-1">
        ...
        <div className="relative w-full aspect-video rounded-3xl border border-border/10 bg-zinc-50/50 dark:bg-zinc-900/10 overflow-hidden group">
          ...
          <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="px-2 py-1 rounded-md bg-background/80 backdrop-blur-sm border border-border/10 text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
              Live Preview
            </div>
          </div>
        </div>
        ...
        <section className="w-full">
          <article className="prose prose-zinc dark:prose-invert 
            ...
            prose-p:text-[13px] prose-p:leading-relaxed prose-p:text-muted-foreground/60 prose-p:font-light
            ...
            prose-code:text-foreground prose-code:font-mono prose-code:text-[11px] prose-code:bg-muted/30 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-md prose-code:before:content-none prose-code:after:content-none
            max-w-none">
            {mdxContent ? (
              <mdxContent.Content components={{ ...getMDXComponents(), ComponentPreview: () => null, h1: () => null }} />
            ) : (
              <p className="text-muted-foreground text-xs italic">No documentation available.</p>
            )}
          </article>
        </section>
        ...
        <footer className="mt-12 pt-8 border-t border-border/10 flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="text-muted-foreground/20 uppercase tracking-[0.2em] text-[8px] font-bold">Notes</span>
            <p className="text-[11px] text-muted-foreground/40 font-light">Part of the creative sky experimental lab.</p>
          </div>
